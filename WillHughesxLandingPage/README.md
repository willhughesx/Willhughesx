...
## If the automation video shows a player but won't play

This almost always means the file didn't actually reach the server at the expected path. Check:
1. The file `assets/videos/automation-walkthrough.mp4` exists in your GitHub repo (not just locally) — large files are sometimes accidentally excluded by a `.gitignore` rule.
2. The path is exact and case-sensitive: `assets/videos/automation-walkthrough.mp4`, all lowercase.
3. After deploying, open the file URL directly in a browser tab (e.g. `https://yoursite.netlify.app/assets/videos/automation-walkthrough.mp4`) — it should download or play on its own. If that URL 404s, the file isn't deployed, and the fix is on the hosting/repo side, not the code.
